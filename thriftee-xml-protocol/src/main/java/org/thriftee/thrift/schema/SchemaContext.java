...
public final class SchemaContext {
  ...
  interface SchemaContextCreatedListener {
    void schemaContextCreated(SchemaContext ctx) throws SchemaBuilderException;
  }
  ...
  SchemaType resolveReference(SchemaReference reference) {
    final SchemaType resolved;
  ...
  SchemaType resolveType(final SchemaType type) {
    return resolveType(type.getModuleName(), type.getTypeName());
  }

  SchemaType resolveType(String moduleName, String typeName) {
    final ModuleSchema module = resolveModule(moduleName);
    SchemaType result = module.getStructs().get(typeName);
    if (result == null) {
